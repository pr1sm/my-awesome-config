...
local function worker(args)
  ...
  -- Settings
  local interfaces = args.interfaces or {"wlan0"}
  local ICON_DIR = awful.util.getdir("config").."/net/icons/"
    ...
    if connected then
      for _, i in pairs(interfaces) do
        ...
        for line in f:lines() do
          -- inet 192.168.0.13/24 brd 192.168.0.255 scope global wlan0
          inet = string.match(line, "inet (%d+%.%d+%.%d+%.%d+)") or inet
          -- link/ether 7c:b0:c2:36:4d:c2 brd ff:ff:ff:ff:ff:ff
